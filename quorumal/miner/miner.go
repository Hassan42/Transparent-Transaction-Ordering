...
func New(eth Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(block *types.Block) bool, publicKey string) *Miner {
	miner := &Miner{
		eth:     eth,
		mux:     mux,
		engine:  engine,
		...
		worker:  newWorker(config, chainConfig, engine, eth, mux, isLocalBlock, true, publicKey),
	}
	go miner.update()
